...
model_xS_R2<-TVAR(dane_monet,lag=p_lag,nthresh=1,model="TAR",thVar=sent,trim=0.2)
down<-model_xS_R2$model.specific$Thres[1]
model_xS_R22<-TVAR(dane_monet[c(1,1,na.omit(regime(model_xS_R2)))>1,],trim=0.45,lag=p_lag,nthresh=1,model="TAR",thVar=-sent[c(1,1,na.omit(regime(model_xS_R2)))>1])
...
model_xS_R2<-TVAR(dane_monet,lag=p_lag,nthresh=1,model="TAR",thVar=pengab,trim=0.2)
pdown<-model_xS_R2$model.specific$Thres[1]
model_xS_R22<-TVAR(dane_monet[c(1,1,na.omit(regime(model_xS_R2)))>1,],trim=0.2,lag=p_lag,nthresh=1,model="TAR",thVar=-pengab[c(1,1,na.omit(regime(model_xS_R2)))>1])
pup<-(-model_xS_R22$model.specific$Thres[1])
model_xS_R3<-TVAR(dane_monet,lag=p_lag,nthresh=2,model="TAR",thVar=pengab,trim=0.2)
...
podzial<-model_split(model_xS_R2,md,reg_xS_R2[-c(1:p_lag)],Amtx,Bmtx,"COM",2)
mddw_xS_R2<-podzial$down
mdup_xS_R2<-podzial$middle

podzial<-model_split(model_xS_R3,md,reg_xS_R3[-c(1:p_lag)],Amtx,Bmtx,"COM")
mdup_xS_R3<-podzial$up
mdmd_xS_R3<-podzial$middle
mddw_xS_R3<-podzial$down

podzial<-model_split(model_xS_R3M,md,zzz[-c(1:p_lag)],Amtx,Bmtx,"COM")
mdup_xS_R3M<-podzial$up
mdmd_xS_R3M<-podzial$middle
mddw_xS_R3M<-podzial$down


podzial<-model_split(model_xS_R3P,md,zzzp[-c(1:p_lag)],Amtx,Bmtx,"COM")
mdup_xS_R3P<-podzial$up
mdmd_xS_R3P<-podzial$middle
mddw_xS_R3P<-podzial$down


podzial<-model_split(model_oS,md,reg_oS[-c(1:p_lag)],Amtx,Bmtx,"COM")
mdup_oS<-podzial$up
...
input_model_B<-list(md,mdup_xS_R3M,mdmd_xS_R3M,mddw_xS_R3M)
...
par(mfrow=c(1,2))
plot(sent)
abline(model_xS_R3$model.specific$Thres[1],0,col="red")
abline(model_xS_R3$model.specific$Thres[2],0,col="blue")
...
plot_own_irf(input_model_R2,c("wibor","gdp"),"Monetary impulse on GDI - TVAR #2")
...
plot_own_irf(input_model_B,c("wibor","cpi"),"Monetary impulse on CPI - TVAR #3 manual")
#plot_own_irf(input_model_B,c("pkb","cpi"),"GDP impulse on CPI - TVAR #3 manual")
plot_own_irf(input_model_B,c("wibor","pkb"),"Wibor impulse on GDP - TVAR #3 manual")
plot_own_irf(input_model_B,c("wibor","reer"),"Wibor impulse on REER - TVAR #3 manual")
