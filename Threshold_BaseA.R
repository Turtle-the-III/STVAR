...
model_xS_R2<-TVAR(dane_monet,lag=p_lag,nthresh=1,model="TAR",thVar=sent,trim=0.01)
down<-model_xS_R2$model.specific$Thres[1]
model_xS_R22<-TVAR(dane_monet[c(1,1,na.omit(regime(model_xS_R2)))>1,],lag=p_lag,nthresh=1,model="TAR",thVar=-sent[c(1,1,na.omit(regime(model_xS_R2)))>1],trim=0.11)
...
podzial<-model_split(model_xS_R2,md,reg_xS_R2[-c(1:p_lag)],Amtx,Bmtx,"EA",2)
mddw_xS_R2<-podzial$down
mdup_xS_R2<-podzial$middle

podzial<-model_split(model_xS_R3,md,reg_xS_R3[-c(1:p_lag)],Amtx,Bmtx,"EA")
mdup_xS_R3<-podzial$up
mdmd_xS_R3<-podzial$middle
mddw_xS_R3<-podzial$down

podzial<-model_split(model_xS_R3M,md,zzz[-c(1:p_lag)],Amtx,Bmtx,"EA")
mdup_xS_R3M<-podzial$up
mdmd_xS_R3M<-podzial$middle
mddw_xS_R3M<-podzial$down

podzial<-model_split(model_xS_R3P,md,zzzp[-c(1:p_lag)],Amtx,Bmtx,"EA")
mdup_xS_R3P<-podzial$up
mdmd_xS_R3P<-podzial$middle
mddw_xS_R3P<-podzial$down

podzial<-model_split(model_oS,md,reg_oS[-c(1:p_lag)],Amtx,Bmtx,"EA")
mdup_oS<-podzial$up
...
input_model_A<-list(md,mdup_xS_R3M,mdmd_xS_R3M,mddw_xS_R3M)
